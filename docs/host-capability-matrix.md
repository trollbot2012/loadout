...
- Proof (headless `codex exec`, 2026-09-02): run 1 without a skill: edit denied, Stop blocked 8 times then released by the (since removed) cap, README untouched; with the cap gone the block repeats until the operator ends the session. Run 2 with `$planning-with-files`: skill read counted, edit allowed, Stop blocked with review missing until the review skill was read.
